...
func ConnectDatabase() *DB {
	...
	dsn := fmt.Sprintf("%v:%v@tcp(%v:%v)/%v?charset=%v&parseTime=%v&loc=%v",
		configuration.Conf.Database.Username,
		configuration.Conf.Database.Password,
		configuration.Conf.Database.Host,
		configuration.Conf.Database.Port,
		configuration.Conf.Database.Name,
		configuration.Conf.Database.Charset,
		configuration.Conf.Database.ParsTime,
		configuration.Conf.Database.Loc,
	)
	// dsn_postgres := fmt.Sprintf("host=%v port=%v user=%v password=%v dbname=%v sslmode=%v TimeZone=%v",
